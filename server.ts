...
app.get("/api/listings", async (req: express.Request, res: express.Response) => {
  let sheetUrl = (req.query.sheetUrl as string) || getStoredGoogleSheetUrl();
  if (sheetUrl && sheetUrl.trim().startsWith("http")) {
    const stored = getStoredGoogleSheetUrl();
    if (!stored || stored.trim() !== sheetUrl.trim()) {
      saveStoredGoogleSheetUrl(sheetUrl.trim());
