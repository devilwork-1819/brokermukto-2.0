...
async function fetchWithTimeout(resource: string, options: any = {}, timeout = 4000) {
...
try {
  ...
  if (fs.existsSync(firebaseConfigPath)) {
    ...
    db = dbId && dbId !== "(default)" ? getFirestore(appInstance, dbId) : getFirestore(appInstance);
  ...
  try {
    ...
    if (fast2smsKey && fast2smsKey !== "MY_FAST2SMS_KEY" && fast2smsKey.trim() !== "") {
      ...
      try {
        ...
        const smsRes = await fetchWithTimeout("https://www.fast2sms.com/dev/bulkV2", {
          method: "POST",
          headers: {
        ...
        if (smsData && smsData.return === true) {
          ...
          const getResOtp = await fetchWithTimeout(getUrlOtp, { method: "GET" }, 4000);
          ...
          if (getDataOtp && getDataOtp.return === true) {
            ...
            const getResQuick = await fetchWithTimeout(getUrlQuick, { method: "GET" }, 4000);
...
async function startServer() {
  // Load system config from Firestore globals document if active without blocking start
  loadSystemConfigFromFirestore().catch(err => {
    console.error("[Firebase] Error loading system config on startup:", err);
  });
