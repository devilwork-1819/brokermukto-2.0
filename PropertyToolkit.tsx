...
export default function PropertyToolkit({ lang }: PropertyToolkitProps) {
  ...
  return (
    <section id="premium-property-toolkit" className="bg-slate-900/80 border border-yellow-500/15 rounded-2.5xl p-4 sm:p-5 md:p-6 shadow-2xl relative overflow-hidden text-white font-sans max-w-5xl mx-auto w-full transition-all">
      {/* Background graphic elements */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-yellow-500/5 rounded-full blur-2xl pointer-events-none"></div>
      
      {/* Header and Title */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-b border-white/5 pb-4.5 mb-5">
        <div className="flex items-center gap-3">
          ...
          <div className="text-center sm:text-left">
            <h3 className="text-base sm:text-lg font-black tracking-tight text-yellow-500 uppercase">
        ...
        <div className="flex flex-wrap items-center justify-center gap-1.5 bg-black/45 p-1 rounded-xl border border-white/5">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer select-none flex items-center gap-1 ${
              activeTab === 'stamp' ? 'bg-yellow-500 text-teal-950 shadow-md' : 'text-white/60 hover:text-white hover:bg-white/5'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer select-none flex items-center gap-1 ${
              activeTab === 'valuation' ? 'bg-yellow-500 text-teal-950 shadow-md' : 'text-white/60 hover:text-white hover:bg-white/5'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer select-none flex items-center gap-1 ${
              activeTab === 'loan' ? 'bg-yellow-500 text-teal-950 shadow-md' : 'text-white/60 hover:text-white hover:bg-white/5'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer select-none flex items-center gap-1 ${
              activeTab === 'verify' ? 'bg-yellow-500 text-teal-950 shadow-md' : 'text-white/60 hover:text-white hover:bg-white/5'
            }`}
          >
          ...
          <div className="md:col-span-7 bg-black/35 border border-white/5 p-4 rounded-2xl flex flex-col justify-between space-y-4">
            <div>
              <div className="flex justify-between items-center mb-1.5">
                <label className="text-xs font-bold text-white/80">{lang === 'bn' ? 'সম্পত্তির আনুমানিক মূল্য বা সরকারি দর:' : 'Property Valuation or Circle Rate:'}</label>
                <span className="text-xs font-black text-yellow-400 bg-yellow-500/10 px-2 py-0.5 rounded-md border border-yellow-500/20">{formatINR(stampValue)}</span>
              </div>
              <input
                type="range"
                min="200000"
              ...
              />
              <div className="flex justify-between text-[10px] text-white/40 font-bold mt-1">
                <span>{lang === 'bn' ? '₹২ লাখ' : '₹2 Lakh'}</span>
                <span>{lang === 'bn' ? '₹৭৫ লাখ' : '₹75 Lakh'}</span>
                <span>{lang === 'bn' ? '₹১.৫ কোটি+' : '₹1.5 Crore+'}</span>
              </div>
            </div>
      ...
      {activeTab === 'valuation' && (
        <div id="property-valuation-tab" className="grid grid-cols-1 md:grid-cols-12 gap-5 items-stretch animate-fade-in">
          {/* Input block */}
          <div className="md:col-span-7 bg-black/35 border border-white/5 p-4 rounded-2xl space-y-3.5">
            <div className="grid grid-cols-2 gap-3.5">
              {/* Type selector */}
              <div>
                <label className="block text-xs font-bold text-white/70 mb-1.5">{lang === 'bn' ? 'সম্পত্তির ধরণ:' : 'Property Type'}</label>
                <select
                  value={propType}
                  onChange={(e) => setPropType(e.target.value)}
                  className="w-full bg-slate-900 border border-white/10 rounded-xl px-2.5 py-2 text-xs font-bold text-white outline-none cursor-pointer focus:border-yellow-500"
                >
              ...
              {/* District locator */}
              <div>
                <label className="block text-xs font-bold text-white/70 mb-1.5">{lang === 'bn' ? 'পশ্চিমবঙ্গের জেলা:' : 'Select District'}</label>
                <select
                  value={valDistrict}
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="col-span-2">
                <label className="block text-xs font-bold text-white/70 mb-1.5">{lang === 'bn' ? 'রাস্তা সংযোগ ব্যবস্থা:' : 'Street/Road Width'}</label>
                <select
                  value={roadType}
              ...
              {/* Size details */}
              <div>
                <label className="block text-xs font-bold text-white/70 mb-1.5">{lang === 'bn' ? 'পরিমাণ:' : 'Size Value'}</label>
                <input
                  type="number"
                  min="1"
                  value={propSize}
                  onChange={(e) => setPropSize(Math.max(1, Number(e.target.value)))}
                  className="w-full bg-slate-900 border border-white/10 rounded-xl px-2.5 py-2 text-xs font-bold text-white outline-none focus:border-yellow-500"
                />
              </div>
            </div>
            ...
            {/* Area Unit toggler */}
            <div>
              <label className="block text-xs font-bold text-white/70 mb-1.5">{lang === 'bn' ? 'পরিমাপের একক:' : 'Measurement Unit'}</label>
          ...
          <div className="md:col-span-5 bg-gradient-to-b from-slate-950 to-slate-900 border-2 border-yellow-500/25 p-5 rounded-2xl flex flex-col justify-between space-y-4">
            ...
            {estimationResult && (
              <div className="text-center my-auto space-y-2.5">
                <div className="text-[10px] text-white/50 uppercase font-bold tracking-widest">{lang === 'bn' ? 'তথা আনুমানিক যুক্তিগ্রাহ্য পরিসীমা' : 'Flipped Fair Market Range'}</div>
                <div className="text-xl sm:text-2xl font-black text-yellow-400 leading-none">
                  {formatINR(estimationResult.low)} - {formatINR(estimationResult.high)}
                </div>
                
                <div className="text-[10px] bg-yellow-500/10 text-yellow-500 px-3 py-1.5 rounded-md inline-block animate-pulse font-bold border border-yellow-500/20">
                  {lang === 'bn' ? 'সম্পূর্ণ ব্রোকারেজ বা দালাল মুক্ত এস্টিমেট' : 'Direct Owner - Pure Broker Free Pricing'}
                </div>
              </div>
            )}

            <p className="text-[9px] text-white/40 leading-normal text-center italic bg-black/40 p-2 rounded-lg mt-2">
              {lang === 'bn' 
                ? 'দ্রষ্টব্য: এই দরটি পশ্চিমবঙ্গের সংশ্লিষ্ট জেলার গড় জমির মার্কেট ট্রেন্ড, এলাকা প্রকার ও রাস্তার সংযোগ সুবিধার ওপর ভিত্তি করে আনুমানিকভাবে গণনা করা হয়েছে। রেজিস্ট্রি অফিসে ডিড করার আগে সরকারি মৌজা অনুযায়ী সার্কেল রেট মিলিয়ে দেখা বাঞ্ছনীয়।' 
