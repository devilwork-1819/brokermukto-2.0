...
// --- In-app browser (WhatsApp/Facebook/Instagram WebView) detection ---
function isInAppBrowser() {
  const ua = navigator.userAgent || '';
  return /FBAN|FBAV|Instagram|WhatsApp|Line|Snapchat|Twitter/i.test(ua);
}
...
export default function SellerFormView({ lang, onBack, onSubmit, activeMobiles }: SellerFormViewProps) {
  ...
  const detectPhoneNumberInMedia = (fileName: string, contentStr?: string): boolean => {
    const phonePattern = /(?:\+91[\s\-]?)?[6-9]\d{9}|(?:\+880[\s\-]?)?01[3-9]\d{8}/;
    
    if (phonePattern.test(fileName)) {
      return true;
    }
    if (contentStr && !contentStr.startsWith('data:')) {
      if (phonePattern.test(contentStr)) {
        return true;
      }
    }
    return false;
  ...
  const handleVideoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (detectPhoneNumberInMedia(file.name)) {
        setErrorMsg(
          lang === 'bn'
            ? '❌ ভিডিও ফাইলে মোবাইল নম্বর সনাক্ত হয়েছে! সরাসরি নম্বর আদান-প্রদান রোধে ব্রোকারমুক্ত-তে ফোন নম্বরযুক্ত মিডিয়া আপলোড নিষিদ্ধ।'
            : '❌ A mobile number was detected in the video file name! Directly exchanging phone numbers through media is strictly prohibited.'
        );
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            // Flag as breach if filename contains actual mobile numbers
            const containsMobileInVideoName = detectPhoneNumberInMedia(file.name);
            
            if (containsMobileInVideoName) {
              setErrorMsg(
                lang === 'bn'
                  ? '❌ ভিডিওতে মোবাইল নম্বর বলা বা টেক্সটে নম্বর দেওয়া সনাক্ত করা গেছে! সরাসরি যোগাযোগ বিনিময় রোধ করতে এটি আপলোড করা সম্ভব নয়।'
  ...
  const handlePhotosChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    files.slice(0, 5).forEach((file: File) => {
      if (detectPhoneNumberInMedia(file.name)) {
        setErrorMsg(
          lang === 'bn'
            ? '❌ আপনার ছবির ফাইলে মোবাইল নম্বর পাওয়া গেছে! ছবি আপলোড বাতিল করা হলো।'
            : '❌ Mobile number was detected in your photo filename! Directly exchanging contact numbers is restricted.'
        );
        window.scrollTo({ top: 0, behavior: 'smooth' });
      ...
      reader.onload = (event) => {
        if (event.target?.result) {
          ...
          if (containsPhone) {
            setErrorMsg(
              lang === 'bn'
                ? '⚠️ আপনার একটি ছবিতে মোবাইল নম্বর সনাক্ত হয়েছে! দয়া করে এটি রিমুভ করুন এবং ফোন নম্বরবিহীন ছবি পুনরায় আপলোড করুন।'
                : '⚠️ A mobile number was detected in this photo overlay scan! Please remove it and upload a clean photo.'
            );
            window.scrollTo({ top: 0, behavior: 'smooth' });
            if (photoInputRef.current) photoInputRef.current.value = '';
          } else {
  ...
  const handleFormSubmit = () => {
    ...
    if (!district) {
      setErrorMsg(lang === 'bn' ? '⚠️ অনুগ্রহ করে একটি জেলা নির্বাচন করুন।' : '⚠️ Please select a district.');
      setCurrentStep(1);
      setTimeout(() => {
    ...
    if (!po || !po.trim()) {
      setErrorMsg(lang === 'bn' ? '⚠️ অনুগ্রহ করে একটি পোস্ট অফিস নির্বাচন করুন।' : '⚠️ Please select a post office.');
      setCurrentStep(1);
      setTimeout(() => {
        if (poRef.current) {
          poRef.current.focus();
      ...
      setErrorMsg(lang === 'bn' ? '⚠️ অনুগ্রহ করে ম্যানুয়ালি পোস্ট অফিসের নাম লিখুন।' : '⚠️ Please write the post office name manually.');
      setCurrentStep(1);
      setTimeout(() => {
        if (customPoRef.current) {
          customPoRef.current.focus();
          customPoRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
      }, 100);
      setTimeout(() => setErrorMsg(null), 5000);
      return;
    ...
    if (!size || !size.trim()) {
      setErrorMsg(lang === 'bn' ? '⚠️ অনুগ্রহ করে জমির পরিমাণ বা সাইজ উল্লেখ করুন।' : '⚠️ Please enter the property size.');
      setCurrentStep(1);
      setTimeout(() => {
        if (sizeRef.current) {
    ...
    if (!type) {
      setErrorMsg(lang === 'bn' ? '⚠️ অনুগ্রহ করে সম্পত্তির ধরণ নির্বাচন করুন।' : '⚠️ Please select a property type.');
      setCurrentStep(1);
      setTimeout(() => {
        if (typeRef.current) {
    ...
    if (!price || !price.trim()) {
      setErrorMsg(lang === 'bn' ? '⚠️ অনুগ্রহ করে সম্পত্তির মূল্য উল্লেখ করুন।' : '⚠️ Please enter the property price.');
      setCurrentStep(1);
      setTimeout(() => {
        if (priceRef.current) {
    ...
    // 2. Validate Step 3 fields (Mobile, compliance checks)
    ...
    if (!agreedDisclaimer) {
      setErrorMsg(lang === 'bn' ? '⚠️ অনুগ্রহ করে এগিয়ে যাওয়ার আগে দায়িত্বমুক্তির ঘোষণা ও শর্তাবলীতে সম্মত হন।' : '⚠️ Please agree to the disclaimer terms below to proceed.');
      setCurrentStep(3);
      setTimeout(() => {
  ...
  return (
    <div id="seller-form-view" className="w-full py-6 md:py-10">
      ...
      {/* In-App Browser (WhatsApp/FB/Instagram) Warning Banner */}
      {showWebviewWarning && (
        <div
          ref={warningRef}
          className="bg-yellow-500/15 border border-yellow-500 text-yellow-200 rounded-2xl p-4 mb-6 text-sm font-bold text-center"
        >
          <div>
            ⚠️{' '}
            {lang === 'bn'
              ? 'আপলোড সঠিকভাবে কাজ করার জন্য, দয়া করে এই পেজটি Chrome ব্রাউজারে খুলুন।'
              : 'For uploads to work properly, please open this page in Chrome instead of this in-app browser.'}
          </div>
          <div className="mt-3">
            <button
              type="button"
              onClick={openInChrome}
              className="px-4 py-2 bg-yellow-500 text-teal-950 rounded-lg text-xs font-black cursor-pointer hover:bg-yellow-400 transition-colors"
            >
              {lang === 'bn' ? 'Chrome-এ খুলুন' : 'Open in Chrome'}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="bg-slate-900/60 border border-white/5 rounded-3xl p-4 md:p-6 backdrop-blur-md">
          <div className="relative flex items-center justify-between">
            ...
            {[
              ...
              { step: 2, icon: '📸', label: lang === 'bn' ? 'মিডিয়া' : 'Media', title: lang === 'bn' ? '২. আপলোড' : '2. Media Gallery' },
              ...
              return (
                <div key={item.step} className="relative z-10 flex flex-col items-center">
                  <button
                    ...
                    className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center font-extrabold text-sm sm:text-base border-2 transition-all duration-300 cursor-pointer ${
                      isActive
                        ? 'bg-slate-950 border-yellow-500 text-yellow-400 shadow-[0_0_20px_rgba(234,179,8,0.5)] scale-105 font-black'
                        : isCompleted
                          ? 'bg-yellow-500 border-yellow-500 text-teal-950 shadow-[0_0_15px_rgba(234,179,8,0.3)] hover:scale-105'
                          : 'bg-slate-950 border-white/20 text-white/70 hover:border-yellow-500/30 hover:scale-105'
                    }`}
                  >
                  ...
                  <div className="text-center mt-2.5">
                    <span className={`block text-[9px] sm:text-xs font-black uppercase tracking-wider ${
                      isActive ? 'text-yellow-500' : isCompleted ? 'text-white/85' : 'text-white/50'
                    }`}>
                      {item.title}
                    </span>
                  </div>
        ...
        <div id="step-1-container" className="space-y-6 max-w-4xl mx-auto pb-20 animate-fade-in">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900/95 border-2 border-white/10 rounded-3xl p-5 md:p-6 shadow-xl hover:shadow-[0_15px_30px_rgba(0,0,0,0.5)] transition-all duration-300 flex flex-col gap-5 hover:border-yellow-500/10">
              ...
              <div className="pt-3 border-t border-white/5">
                <label className="block text-xs font-bold text-white/70 mb-1.5">
                  📝 {lang === 'bn' ? 'বিশেষ মন্তব্য (Special Remarks)' : 'Special Remarks'}
                </label>
                <textarea
                  ...
                  placeholder={
                    lang === 'bn'
                      ? 'সম্পত্তির ব্যাপারে কোনো অতিরিক্ত তথ্য থাকলে এখানে লিখুন (যেমন: জলের লাইন, গাছপালা, রাস্তার পাশে ইত্যাদি)...'
                      : 'Type/write any additional details about your property (e.g., water source, trees, road side frontage, etc.)...'
                  }
                  rows={2}
                  maxLength={500}
                  className="w-full bg-black/35 hover:bg-black/45 border border-white/15 focus:border-yellow-500 rounded-xl px-3.5 py-2 text-sm font-semibold text-white outline-none placeholder-white/30 transition-colors resize-none"
                />
                <div className="text-right text-[9px] text-white/30 mt-1 font-semibold leading-none">
                  {specialRemarks.length}/500
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end pt-4">
            ...
            >
              <span>{lang === 'bn' ? 'চলুন পরবর্তী: মিডিয়া আপলোড ➔' : 'Next Step: Upload Media ➔'}</span>
            </button>
          </div>
        </div>
      )}
        ...
        <div id="step-2-container" className="max-w-3xl mx-auto pb-20 space-y-6 animate-fade-in">
          ...
          <div className="bg-slate-900/95 border-2 border-white/10 rounded-3xl p-5 md:p-6 shadow-xl hover:shadow-[0_15px_30px_rgba(0,0,0,0.5)] transition-all duration-300 flex flex-col gap-6 hover:border-yellow-500/10">
              ...
              {videoUrl ? (
                ...
                >
                  ...
                  <span className="text-[10px] text-yellow-500/90 font-bold bg-yellow-500/10 border border-yellow-500/20 px-3 py-1 rounded-full mt-1">
                    ⚠️ {lang === 'bn' ? 'দয়া করে কোনো মোবাইল নম্বর মুখে উচ্চারণ না করে ভিডিওটি আপলোড করুন।' : 'Please upload video without speaking any mobile number.'}
                  </span>
                </div>
              )}
              <input 
            ...
            <div>
              ...
              >
                ...
                <span className="text-[10px] text-yellow-500/90 font-bold bg-yellow-500/10 border border-yellow-500/20 px-3 py-1 rounded-full mt-1">
                  ⚠️ {lang === 'bn' ? 'দয়া করে ছবিতে কোনো মোবাইল নম্বর প্রদর্শন না করে আপলোড করুন।' : 'Please use photos without showing any mobile number.'}
                </span>
              </div>
              <input 
                type="file" 
          ...
          <div className="flex justify-between items-center gap-4 pt-4">
            <button
              ...
              className="px-5 py-3.5 bg-slate-800 hover:bg-slate-700 text-yellow-500 hover:text-yellow-400 border-2 border-yellow-500/30 hover:border-yellow-500/60 rounded-2xl text-xs font-black transition-all duration-150 transform hover:-translate-y-[2px] active:translate-y-[2px] shadow-[0_4px_0_#78350f] hover:shadow-[0_6px_0_#78350f] active:shadow-none flex items-center gap-2 cursor-pointer select-none group uppercase tracking-wider"
            >
              <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
              <span>{lang === 'bn' ? 'বিবরণীতে ফিরুন' : 'Back to Details'}</span>
            </button>
            <button
              type="button"
      ...
      {currentStep === 3 && (
        <div id="step-3-container" className="max-w-2xl mx-auto pb-20 space-y-6 animate-fade-in">
            ...
            <div className="space-y-4">
              <div className="bg-yellow-500/5 border border-yellow-500/25 rounded-2xl p-4.5 mt-5 animate-scale-up">
                <div className="text-xs text-yellow-500 font-extrabold flex items-center gap-1.5 mb-2">
                  <span>⚠️</span>
                  <span>{lang === 'bn' ? 'সতর্কীকরণ ও বিক্রয় শর্তাবলী' : 'Warning & Seller Terms'}</span>
                </div>
                <p className="text-[11px] text-white/60 leading-relaxed font-semibold mb-3">
                  {lang === 'bn' 
                    ? 'আমি সম্মত হচ্ছি যে BrokerMukto-তে সম্পত্তি তালিকাভুক্তি ও ক্রেতাদের সাথে আমার সমস্ত চুক্তি বা যোগাযোগ সম্পূর্ণ আমার নিজস্ব ঝুঁকিতে সম্পাদিত হবে। প্ল্যাটফর্ম কোনো আর্থিক লেনদেন বা কাগজ চেক করার ব্যাপারে কোনো দায় বহন করে না।'
                ...
                <label className="flex items-start gap-2.5 cursor-pointer select-none">
                  ...
                  <span className="text-xs font-black text-white/80 hover:text-white transition-colors">
                    {lang === 'bn' ? 'আমি এই ঘোষণা ও দায়িত্বমুক্তির শর্তাবলীতে সম্মত আছি' : 'I accept the disclaimer and release of liability'}
                  </span>
                </label>
              </div>
              ...
              <div className="bg-blue-500/5 border border-blue-500/25 rounded-2xl p-4.5 animate-scale-up">
                ...
                <p className="text-[11px] text-white/60 leading-relaxed font-semibold mb-3">
                  {lang === 'bn' 
                    ? 'আমি অত্যন্ত গুরুত্বের সাথে ঘোষণা করছি যে এই সম্পত্তিটি পশ্চিমবঙ্গ রিয়েল এস্টেট রেগুলেটরি অথরিটি (WBRERA) নির্দেশাবলী এবং ভারতীয় সম্পত্তি আইনের অধীনে সম্পূর্ণ আইনগ্রাহ্য। সম্পত্তিটিতে কোনো আইনি বিরোধ বা কোলাটেরাল নেই এবং এটির বিক্রির সম্পূর্ণ আইনগত অধিকার আমার আছে।'
                ...
                <label className="flex items-start gap-2.5 cursor-pointer select-none">
                  ...
                  />
                  <span className="text-xs font-black text-white/80 hover:text-white transition-colors">
                    {lang === 'bn' ? 'আমি এ মর্মে আইনগত সত্যতা ঘোষণা করছি ও দায়ভার গ্রহণ করছি' : 'I certify the legal ownership and WBRERA compliance'}
                  </span>
                </label>
              </div>
            </div>
          ...
          <div className="flex justify-start pt-4">
            ...
            >
              <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
              <span>{lang === 'bn' ? 'মিডিয়ায় ফিরুন' : 'Back to Media'}</span>
            </button>
          </div>
        </div>
      )}
