...
import { ArrowLeft, Search, Filter, Share2, MapPin, CheckCircle, Video, Lock, Phone, MessageSquare, ExternalLink, X, Key, Home, Sprout, Store, Trees, Waves, Building, Mic, MicOff } from 'lucide-react';
...
interface ListingsViewProps {
  ...
  onDeleteListing: (id: number, mobile: string) => Promise<boolean> | boolean;
...
export default function ListingsView({
  ...
  const handleVoiceSearch = () => {
    ...
    if (!SpeechRecognition) {
      alert(lang === 'bn' ? 'দুঃখিত, আপনার ব্রাউজার ভয়েস সার্চ সাপোর্ট করে না।' : 'Sorry, your browser does not support Voice Search.');
      return;
    }
    ...
    recognition.lang = lang === 'bn' ? 'bn-IN' : 'en-IN';
  ...
  const handleApplyRecentSearch = (search: RecentSearch) => {
    ...
    else if (search.filterPrice === 'hundred_up') setSliderMaxPrice(50000000);
  };
  ...
  const handleVerifyOtpAndExecute = async () => {
    ...
    try {
      ...
      if (verifyMode === 'delete') {
        const success = await onDeleteListing(target.id, trimmedMobile);
        if (success) {
        ...
        <div className="w-full grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
          ...
          <div className="relative col-span-2 sm:col-span-2 lg:col-span-2">
            ...
            <input 
              ...
              placeholder={isListening ? (lang === 'bn' ? 'বলুন, শুনছি...' : 'Speak now, listening...') : t.searchPh}
              className={`w-full bg-black/30 hover:bg-black/40 border ${isListening ? 'border-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]' : 'border-white/10 focus:border-yellow-500'} rounded-xl py-2 pl-9 pr-10 text-xs md:text-sm font-semibold text-white outline-none placeholder-white/35 transition-colors`}
            />
            <button
              type="button"
              onClick={handleVoiceSearch}
              className={`absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-lg transition-all cursor-pointer flex items-center justify-center ${
                isListening 
                  ? 'bg-red-500 text-white animate-pulse shadow-md shadow-red-500/20' 
                  : 'text-white/40 hover:text-yellow-400 hover:bg-white/5 active:scale-95'
              }`}
            ...
            {voiceError && (
              <div className="absolute left-0 right-0 top-full mt-1.5 bg-red-950/95 border border-red-500/40 text-red-200 p-2.5 rounded-xl text-[10px] z-50 flex items-center justify-between gap-2 shadow-xl shadow-black/60 backdrop-blur-md">
                <span className="leading-snug flex-1">
                  🛑 {voiceError === 'not-allowed' ? (
                    lang === 'bn' 
                      ? 'মাইক্রোফোন পারমিশন ব্লকড! ভয়েস সার্চ সচল করতে সার্চবারের পাশে বা ব্রাউজার সেটিংসে গিয়ে মাইক্রোফোন পারমিশন দিন।' 
                      : 'Microphone permission blocked! Please grant microphone access in your browser or address bar settings to search with voice.'
                  ) : (
                    lang === 'bn'
                      ? `ভয়েস সার্চ ত্রুটি: ${voiceError}`
                      : `Voice search error: ${voiceError}`
                  )}
                </span>
                <button 
                  type="button" 
                  onClick={() => setVoiceError(null)}
                  className="hover:bg-red-500/25 p-1 rounded text-red-400 transition-colors shrink-0"
                >
          ...
          <div className="flex items-center gap-3 bg-black/40 border border-white/5 px-3 py-1.5 rounded-xl w-full lg:w-auto lg:max-w-md shrink-0">
            <div className="flex flex-col min-w-[100px]">
              ...
              <span className="text-xs font-black text-yellow-500 mt-1 leading-none">
                {sliderMaxPrice >= 50000000 
                  ? (lang === 'bn' ? 'সর্বোচ্চ: ৫ কোটি+' : 'Max: 5 Crores+') 
            ...
            <input 
              ...
              max={50000000}
              step={100000}
              value={sliderMaxPrice > 50000000 ? 50000000 : sliderMaxPrice}
              onChange={(e) => {
                const val = parseInt(e.target.value);
                setSliderMaxPrice(val);
                setPriceFilterMode('slider');
                setFilterPrice('');
              }}
            ...
            <button
              ...
              className={`text-[9px] font-bold p-1 px-2 rounded-md ${
                filterPrice !== '' || sliderMaxPrice < 50000000 || priceFilterMode === 'slider'
              ...
              disabled={filterPrice === '' && sliderMaxPrice >= 50000000 && priceFilterMode === 'bracket'}
            >
              {lang === 'bn' ? 'রিসেট' : 'Reset'}
            </button>
          </div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        className="py-1.5 px-2.5 bg-red-950/20 hover:bg-red-950/45 border border-red-500/15 hover:border-red-500/40 rounded-xl text-[10px] font-black text-red-400 transition-all uppercase tracking-wide flex items-center justify-center gap-1 cursor-pointer select-none active:scale-95"
                  ...
                  {/* Road/Area & Remarks Scrolling Ticker (Marquee) */}
                  <div className="mb-4 bg-slate-950/60 border border-yellow-500/10 rounded-2xl p-2.5 flex items-center gap-2 overflow-hidden select-none" title={lang === 'bn' ? 'হাইলাইট স্ক্রোল (থামাতে মাউস রাখুন)' : 'Highlight Ticker (Hover to Pause)'}>
                    <div className="shrink-0 flex items-center gap-1 px-1.5 py-0.5 bg-yellow-500/15 border border-yellow-500/30 text-yellow-500 text-[9px] font-black uppercase rounded-lg tracking-wider animate-pulse">
                      <span className="w-1.5 h-1.5 rounded-full bg-yellow-400"></span>
                      <span>{lang === 'bn' ? 'হাইলাইট' : 'LIVE'}</span>
                    </div>
                    <div className="flex-1 overflow-hidden relative w-full flex items-center h-5">
                      <div className="animate-marquee whitespace-nowrap text-[11px] font-black text-yellow-400/90 tracking-wide flex items-center gap-6">
                        <span>
                          🛣️ {lang === 'bn' ? 'রাস্তা সংযোগ: ' : 'Road connection: '}
                          <span className="text-white/90 font-bold">{item.road || (lang === 'bn' ? 'তথ্য নেই' : 'N/A')}</span>
                        </span>
                        <span className="text-white/20">•</span>
                        <span>
                          📍 {lang === 'bn' ? 'এলাকা: ' : 'Area: '}
                          <span className="text-white/90 font-bold">{item.po}, {getDistrictLabel(item.district)}</span>
                        </span>
                        {item.specialRemarks && (
                          <>
                            <span className="text-white/20">•</span>
                            <span className="text-yellow-400">
                              ...
                              <span className="text-white/90 font-bold italic">{item.specialRemarks}</span>
                            </span>
                          </>
                        )}
                        {/* Duplicate for smooth continuous wrapping */}
                  ...
                  <div className="space-y-2 mt-4 text-xs font-semibold leading-relaxed border-t border-white/5 pt-4">
                    ...
                    {item.specialRemarks && (
                      <div className="mt-3 bg-white/5 border border-white/5 p-2.5 rounded-2xl">
                        <div className="text-[10px] text-yellow-500 font-extrabold uppercase mb-1 tracking-wider">
                          📌 {lang === 'bn' ? 'বিশেষ মন্তব্য' : 'Special Remarks'}
                        </div>
                        <p className="text-white/80 text-[11px] font-semibold leading-relaxed whitespace-pre-line select-text">
                          {item.specialRemarks}
                        </p>
                      </div>
                    )}
