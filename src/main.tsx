...
if (typeof window !== 'undefined') {
  ...
  // Cache check result to avoid redundant pings and instantly prevent timeouts
  let isStaticSpaModeChecked = false;
  let isStaticSpaMode = localStorage.getItem('bm_static_spa_mode') === 'true';

  // If we are clearly on Vercel, force static SPA mode immediately to avoid slow timeouts or server roundtrips
  const hostname = window.location.hostname;
  if (hostname.endsWith('.vercel.app') || hostname.endsWith('vercel.app')) {
    isStaticSpaMode = true;
    isStaticSpaModeChecked = true;
    localStorage.setItem('bm_static_spa_mode', 'true');
  }
      ...
      if (!isStaticSpaModeChecked) {
        try {
          ...
          if (pingRes.ok && ct.includes("application/json")) {
            ...
            localStorage.setItem('bm_static_spa_mode', 'false');
          } else {
            isStaticSpaMode = true;
            localStorage.setItem('bm_static_spa_mode', 'true');
          }
