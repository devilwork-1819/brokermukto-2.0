  ...
  window.fetch = async function (input, init) {
    ...
    if (url.startsWith('/api/') || url.includes('/api/')) {
      ...
      try {
        ...
        // Engage static sandbox mode ONLY if the server returned an HTML fallback page (e.g. Vercel SPA 404 routing page)
        // If the server is active and returned any JSON error/validation response (like 422, 400), let the client see it.
        if (contentType && contentType.includes("text/html")) {
          console.warn(`[API Proxy Interceptor] Active server response returned HTML text for "${url}". Engaging client-side sandbox mode...`);
  ...
  async function handleStaticFallback(url: string, init?: RequestInit): Promise<Response> {
    ...
    if (urlPath.endsWith('/api/send-otp')) {
      ...
      if (fast2smsKey && fast2smsKey !== "MY_FAST2SMS_KEY" && fast2smsKey !== "") {
        ...
        try {
          // Attempt dispatch with Fallback 2 GET API route=q (Quick SMS) - most reliable direct dispatch!
          ...
          await originalFetch(getUrlQuick, { method: "GET", mode: "no-cors" });
          smsSentReal = true;
          ...
          console.warn(`[Vercel Static Proxy] Browser GET dispatch failed, trying backup route=otp...`, smsErr);
          ...
          try {
            ...
            await originalFetch(getUrlOtp, { method: "GET", mode: "no-cors" });
            smsSentReal = true;
          } catch (otpErr: any) {
            console.error(`[Vercel Static Proxy] Direct browser fallback also failed:`, otpErr);
          }
        }
