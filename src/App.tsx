  ...
  useEffect(() => {
    ...
    const unsubscribeListings = onSnapshot(
      ...
      (snapshot) => {
        ...
        if (items.length === 0) {
          // Seed Firestore with initial listings so they are persisted there
          initialListings.forEach(async (item) => {
            try {
              await setDoc(doc(db, 'listings', String(item.id)), item);
            } catch (err) {
              console.warn('[Firebase Seed] Failed to seed default listing:', err);
            }
          });
          setListings(initialListings);
        } else {
        ...
        <main className={`flex-1 w-full max-w-[1650px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 relative ${view === 'listings' ? 'py-1.5 md:py-2 pb-24 sm:pb-28' : 'py-4 md:py-6 pb-24 sm:pb-28'}`}>
          ...
          {view === 'sell-pay' && (
            <SellerPaymentView 
              ...
              onBack={() => setView(pendingListing ? 'sell' : 'home')}
