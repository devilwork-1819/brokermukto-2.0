...
export default function App() {
  ...
  const fetchFallbackServerListings = (activeUrl: string) => {
    const sheetQuery = activeUrl ? `?sheetUrl=${encodeURIComponent(activeUrl)}` : '';
    fetch(`/api/listings${sheetQuery}`)
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) {
          setListings(data);
          try {
            localStorage.setItem('bm_listings_v3', JSON.stringify(data));
          } catch {}
        }
      })
      .catch(err => console.warn('[Broker Mukto Sync] Server fetch listings failed:', err));
  };

  const syncListingsData = (activeUrl: string) => {
    if (isGoogleSheetCsvUrl(activeUrl)) {
      console.log('[Broker Mukto Sync] Direct CSV runtime fetch from Google Sheet:', activeUrl);
      fetch(activeUrl, { cache: 'no-store' })
        ...
        .then(csvText => {
          const parsed = parseCsvListings(csvText);
          if (parsed && parsed.length > 0) {
            setListings(parsed);
            try {
              localStorage.setItem('bm_listings_v3', JSON.stringify(parsed));
            } catch {}
            console.log(`[Broker Mukto Sync] Direct CSV sync successful: ${parsed.length} listings loaded.`);
          }
        })
        .catch(err => {
          console.warn('[Broker Mukto Sync] Direct CSV fetch listings failed, falling back to server API:', err);
          fetchFallbackServerListings(activeUrl);
        });
    } else {
      fetchFallbackServerListings(activeUrl);
    }
  ...
  useEffect(() => {
    // Listings (initial load from localStorage cache for instant UI rendering)
    try {
    ...
    // Fetch freshest data from Express Server / CSV
    const activeSheetUrl = getGoogleSheetUrl();
    syncListingsData(activeSheetUrl);
  ...
  // Periodic polling interval for real-time multi-device sync (runs every 10 seconds)
  useEffect(() => {
    ...
    const interval = setInterval(fetchListingsPeriodically, 10000);
    return () => clearInterval(interval);
  }, []);

  // Force direct refresh whenever user visits Home or Listings tabs
  useEffect(() => {
  ...
  const saveListings = (updated: Listing[]) => {
    ...
    .catch(err => console.warn('[Broker Mukto Sync] Failed to sync listings to server:', err));
  };
